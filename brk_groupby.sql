...
birlikte kullanılır
YAZILAN SUTUNA GORE SATIRLARI O SUTUNUN ISMINE GORE BIRLESTIRIYOR
*/
...
select isim,sum(urun_miktari) as toplam_urun from manav group by isim --select ile group by ayni sutun olmali
...
select urun_adi,count(isim) as toplam_urun_alan_sayisi from manav where urun_adi is not null
group by urun_adi --gore dendiginde gruplaniyor
 ...
 select urun_adi,max(urun_miktari) as en_fazla_urun,min(urun_miktari) as en_az_urun from manav
 where urun_adi is not null
 group by urun_adi 
 order by en_fazla_urun
...
select isim,urun_adi,sum(urun_miktari) as toplam_urun_miktari from manav where urun_adi is not null
...
select ulke,cinsiyet,round(avg(maas)) as ortalama_maas from personel group by ulke,cinsiyet
...
select ulke ,cinsiyet, count(*) as calisan_sayisi, round(avg(yas),2) as yas_ortalamasi from personel
